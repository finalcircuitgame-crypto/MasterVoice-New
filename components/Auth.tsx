...
        
        if (data.session) {
            // **FIX #1 (Sign Up w/ Auto-Login): Clear loading state and redirect**
            if (isMounted.current) {
                setLoading(false); 
...
        }
        ...
        // **FIX #2 (Sign In Success): Clear loading state and redirect**
...
      // Only clear loading here if we are NOT transitioning to the verify step, 
      // or if an error occurred. The success cases are handled above.
      if (isMounted.current && authStep === 'credentials') setLoading(false);
    }
  };
...
        if (data.session) {
            // **FIX #3 (Verification Success): Clear loading state and redirect**
...
    } finally {
        clearTimeout(safetyTimeout);
        // No need to set loading(false) here, as it's handled in the try/catch blocks
    }
  };
